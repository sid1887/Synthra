...
async function measureCall(fn: () => Promise<any>): Promise<[any, number]> {
...
async function testAdvancedSimulation() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/advanced-simulation/transient?analysisId=sim-001&sampleCount=100')
    );
...
async function testCircuitComparison() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/advanced-simulation/comparison?originalId=circuit-001&modifiedId=circuit-002')
    );
...
async function testParameterSweep() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/advanced-simulation/sweep?componentId=R1&parameterName=Resistance&start=1000&end=10000&steps=20')
    );
...
async function testScene3D() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/scene-3d/visualization?analysisId=scene-001&mode=power')
    );
...
async function testAdaptiveExplanation() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/ai-orchestration/explain?analysisId=analysis-001&expertiseLevel=intermediate')
    );
...
async function testAutomationDashboard() {
  ...
  try {
    const [data, duration] = await measureCall(() =>
      callAPI('/automation-engine/dashboard?circuitId=circuit-001')
    );
