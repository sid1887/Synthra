...
// Validation Helpers
// ============================================================================
function validateRequiredField(obj, field, type) {
    if (!(field in obj)) {
...
function validateAnalysisResponse(obj) {
    ...
    for (const [field, type] of requiredFields) {
        const error = validateRequiredField(obj, field, type);
        if (error)
            errors.push(error);
    }
    ...
    if (obj.circuit) {
        ...
        errors.push(...circuitErrors.map(e => ({
            ...e,
    ...
    if (obj.explanation) {
        ...
        errors.push(...explanationErrors.map(e => ({
            ...e,
...
function validateImagePayload(obj) {
    const errors = [];
...
function validateComponentDetection(obj) {
    const errors = [];
...
function validateCircuitIdentification(obj) {
    const errors = [];
    if (!obj.label)
        errors.push({ field: 'label', message: 'Required' });
    if (!obj.family)
        errors.push({ field: 'family', message: 'Required' });
